...
namespace :db do
  ...
  desc 'Rollback down to specified migration, or
    rollback last STEP=x migrations (default 1)'
  task :rollback, [:version] => :load_migrations do |t, args|
    ...
    if vers
      ...
      # note this is N queries as currently implemented
      applied = migrations.delete_if(&:needs_up?).sort
  ...
  task migrations: :load_migrations do
    puts migrations.sort.reverse.map do |m|
      "#{m.position}  #{m.name}  #{m.needs_up? ? '' : 'APPLIED'}"
    end
  end
  ...
  task :create, [:repository] => :environment do |t, args|
    ...
    database       = config[:database]  || config[:path].sub(%r{\/}, '')
    ...
    when 'mysql'
      query = [
        ...
        "CREATE DATABASE #{database} DEFAULT CHARACTER SET
        #{charset} DEFAULT COLLATE #{collation}".inspect
      ]
  ...
  task :drop, [:repository] => :environment do |t, args|
    ...
    when 'mysql'
      query = [
        'mysql',
        "--user=#{user}", (password.blank? ? '' : "--password=#{password}"),
        (%w(127.0.0.1 localhost).include?(host) ? '-e' : "--host=#{host} -e"),
